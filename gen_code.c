...
void gen_program_start (PROGRAM *p)
{
  ...
  //gen_inst ("sub %,#", REG_SP, LINKAGE_SIZE);
  ...
  //gen_inst ("push %", REG_SP);
  //gen_inst ("sub %,#", REG_SP, 8192);
  gen_inst ("mov [bottom],%", REG_SP);
  gen_inst ("sub dword[bottom],#", 8192);
  //gen_inst ("pop %", REG_SP);
  ...
  //gen_inst ("push %", REG_SP);
  //gen_inst ("add %,#",REG_SP,DYNAMIC_CHAIN);
  ...
  if (offset) {
    ...
    //gen_inst ("ble % % STKOV", REG_SP, REG_S0);
    gen_inst ("cmp %,[bottom]", REG_SP);
    gen_inst ("jle STKOV");
  }
}
...
void gen_program_block_end (PROGRAM *p)
{
  ...
  //gen_inst ("add %,#", REG_SP, LINKAGE_SIZE);
  //gen_inst ("jr %"     , REG_RA);
}
...
void gen_program_end (PROGRAM *p)
{
  ...
  gen_inst ("lea %,[badrngstr]", REG_T1);
  gen_inst ("mov [%+8],%", REG_FP, REG_T1);
  gen_inst ("jmp failure");

  gen ("BADDIV:\n");
  //gen_inst ("la % baddivstr", REG_A0);
  gen_inst ("lea %,[baddivstr]", REG_T1);
  gen_inst ("mov [%+8],%", REG_FP, REG_T1);
  gen_inst ("jmp failure");

  gen ("BADCAS:\n");
  //gen_inst ("la % badcasstr", REG_A0);
  gen_inst ("lea %,[badcasstr]", REG_T1);
  gen_inst ("mov [%+8],%", REG_FP, REG_T1);
  gen_inst ("jmp failure");

  gen ("BADPTR:\n");
  //gen_inst ("la % badptrstr", REG_A0);
  gen_inst ("lea %,[badptrstr]", REG_T1);
  gen_inst ("mov [%+8],%", REG_FP, REG_T1);
  gen_inst ("jmp failure");

  gen ("BADSUB:\n");
  //gen_inst ("la % badsubstr", REG_A0);
  gen_inst ("lea %,[badsubstr]", REG_T1);
  gen_inst ("mov [%+8],%", REG_FP, REG_T1);
  gen_inst ("jmp failure");

  gen ("STKOV:\n");
  //gen_inst ("la % stkovstr", REG_A0);
  gen_inst ("lea %,[stkovstr]", REG_T1);
  gen_inst ("mov [%+8],%", REG_FP, REG_T1);
  gen_inst ("jmp failure");

  gen ("HEAPOV:\n");
  //gen_inst ("la % heapovstr", REG_A0);
  gen_inst ("lea %,[heapovstr]", REG_T1);
  gen_inst ("mov [%+8],%", REG_FP, REG_T1);
  gen_inst ("jmp failure");
  ...
  //gen_inst ("syscall");
  //gen_inst ("jr %", REG_RA);
  gen_inst ("mov %,nlstr", REG_T0);
  gen_inst ("call WriteChar");
  gen_inst ("mov %,%", REG_SP, REG_FP);
  gen_inst ("pop %", REG_FP);
  gen_inst ("ret");
  ...
  gen_inst ("mov %,[%+8]", REG_T1, REG_FP);
  gen_inst ("mov [%+20],%", REG_FP, REG_T1);
  //gen_inst ("sb % 0(%)", REG_A3, REG_A1);
  gen_inst ("mov %,[%+20]", REG_T2, REG_FP);
  gen_inst ("mov [%+12],%", REG_FP, REG_T2);
  //gen_inst ("add % % 1", REG_A0, REG_A0);
  gen_inst ("add word[%+8],#", REG_FP, 1);
  //gen_inst ("add % % 1", REG_A1, REG_A1);
  gen_inst ("add word[%+12],#", REG_FP, 1);
  //gen_inst ("sub % % 1", REG_A2, REG_A2);
  gen_inst ("sub word[%+16],#", REG_FP, 1);
  //gen_inst ("bgtz % copy", REG_A2);
  gen_inst ("cmp word[%+12],#", REG_FP, 0);
  gen_inst ("jg copy");
  //gen_inst ("jr %", REG_RA);
  gen_inst ("mov %,%", REG_SP, REG_FP);
  gen_inst ("pop %", REG_FP);
  ...
  gen_inst ("mov dword[%+12], heapptr", REG_FP);
  //gen_inst ("sub % % %", REG_A0, REG_A1, REG_A0);
  gen_inst ("mov %,[%+8]", REG_T1, REG_FP);
  gen_inst ("mov %,[%+12]", REG_T2, REG_FP);
  gen_inst ("sub %,%", REG_T1, REG_T2);
  //gen_inst ("sw % heapptr", REG_A0);
  gen_inst ("mov %,[%+8]", REG_T1, REG_FP);
  gen_inst ("mov [heapptr],%", REG_T1);
  //gen_inst ("la % heap", REG_A1);
  gen_inst ("lea %,[heap]", REG_T1);
  gen_inst ("mov [%+12],%",REG_FP, REG_T1);
  //gen_inst ("blt % % HEAPOV", REG_A0, REG_A1);
  gen_inst ("mov %,[%+8]", REG_T1, REG_FP);
  gen_inst ("mov %,[%+12]", REG_T2, REG_FP);
  gen_inst ("cmp %,%", REG_T1, REG_T2);
  gen_inst ("jl HEAPOV");
  //gen_inst ("jr %", REG_RA);
  gen_inst ("mov %,%", REG_SP, REG_FP);
  gen_inst ("pop %", REG_FP);
  ...
  gen ("badrngstr db 10,\"Value out of range in assignment$\",10\n");
  //gen ("baddivstr: .asciiz \"\\nDivision by zero\\n\"\n");
  gen ("baddivstr db 10,\"Division by zero$\",10\n");
  //gen ("badcasstr: .asciiz \"\\nValue not handled in case statement\\n\"\n");
  gen ("badcasstr db 10,\"Value not handled in case statement$\",10\n");
  //gen ("badptrstr: .asciiz \"\\nAttempt to use a null pointer\\n\"\n");
  gen ("badptrstr db 10,\"Attempt to use a null pointer$\",10\n");
  //gen ("badsubstr: .asciiz \"\\nSubscript out of bounds\\n\"\n");
  gen ("badsubstr db 10,\"Subscript out of bounds$\",10\n");
  //gen ("stkovstr:  .asciiz \"\\nStack overflow\\n\"\n");
  gen ("stkovstr db 10,\"Stackoverflow$\",10\n");
  //gen ("heapovstr: .asciiz \"\\nOut of heap space\\n\"\n");
  gen ("heapovstr db 10,\"Out of heap space$\",10\n");
...
void gen_assign (EXPR *te, EXPR *se)
{
  ...
  if (tte->tag == TypeSubrange_) {
    ...
    gen_store (sreg, REG_T1, te);
    return;
  }
  if (get_constant_value (se) || tte->size == 4) {
    //int sreg = get_expr_in_reg (se, REG_T9);
    int sreg = get_expr_in_reg (se, REG_T0);
    //gen_store (sreg, REG_T8, te);
    gen_store (sreg, REG_T1, te);
    return;
  }
  if (tte->size > 0) {
    ...
    if (sreg != REG_A0)
      ...
      gen_inst ("mov [%+8],%", REG_FP, sreg);
    treg = get_addr_in_reg (te, REG_A1);
    if (treg != REG_A1)
      //gen_inst ("move % %", REG_A1, treg);
      gen_inst ("mov [%+12],%", REG_FP, treg);
    //gen_inst ("li % #", REG_A2, tte->size);
    gen_inst ("mov [%+16],#", REG_FP, tte->size);
    //gen_inst ("jal copy");
    gen_inst ("call copy");
    return;
  }
...
void gen_stmt_case (STMTCASE *sc)
{
  ...
  for ( ; crlist; crlist = crlist->next) {
    ...
    int creg = get_expr_in_reg (crlist->expr, REG_T2);
...
void gen_stmt_for (STMTFOR *sf)
{
  ...
  if (get_constant_value (fe) && get_constant_value (te)) {
    ...
    if(up) {
        ...
        gen_inst ("jg UPLARGE");
        gen_inst ("mov %,#", REG_T2, 0);
        gen ("UPLARGE:\n");
        gen_inst ("mov %,%", REG_T2, 1);
    }
    else {
        gen_inst ("cmp %,%", ireg, treg);
        gen_inst ("jl DOWNLESS");
        gen_inst ("mov %,#", REG_T2, 0);
        gen ("DOWNLESS:\n");
        gen_inst ("mov %,%", REG_T2, 1);
    }
    //gen_inst ("bnez % @", REG_T9, label_end);
    gen_inst ("cmp %,#", REG_T2, 0);
    gen_inst ("jne @", label_end);
  }
  ...
  /* get the limit value, compare, and branch */
  {
    ...
    ireg = get_expr_in_reg (ierhs, REG_T2);
    ...
    gen_store (ireg, REG_T1, ielhs);

    //treg = get_expr_in_reg (tmprhs, REG_T8);
    treg = get_expr_in_reg (tmprhs, REG_T1);
    //gen_inst ("~ % % %", (up ? "sgt" : "slt"), REG_T9, ireg, treg);
    //gen_inst ("beqz % @", REG_T9, label_start);
    if(up) {
        gen_inst ("cmp %,%", ireg, treg);
        gen_inst ("jle @", label_start);
    }
    else {
        gen_inst ("cmp %,%", ireg, treg);
        gen_inst ("jge @", label_start);
    }
  }
...
void gen_load (int reg, OBJECT *o)
{
  ...
  if (m->mem_plevel == level)
    ...
    gen_inst ("mov %,[%#]", reg, m->mem_plevel-1000, m->mem_offset);
  else
    assert (0);
  if (m->mem_indirect)
...
void gen_store (int reg, int scratch, EXPR *e)
{
  ...
  if (e->reg_name && e->reg_spill == 0) {
    ...
    if (m->mem_indirect) {
      ...
      gen_inst ("mov %,[%#]", scratch, m->mem_plevel-1000, m->mem_offset);
      //gen_inst ("sw % 0(%)", reg, scratch);
      gen_inst ("mov %,%", scratch, reg);
    }
    else
      //gen_inst ("sw % #(%)", reg, m->mem_offset, m->mem_plevel-1000);
      gen_inst ("mov [%#],%", m->mem_plevel-1000, m->mem_offset, reg);
  }
  else
    assert (0);
}
...
int get_expr_in_reg (EXPR *e, int scratch)
{
  ...
  case ObjValue_: {
    ...
    case ValueStr_: {
      ...
      gen_inst ("lea %,[LS#]", scratch, tt->text_number);
      return scratch;
    }
    default:
      break;
...
void gen_expr_binop (int op, EXPR *l, EXPR *r, int reg)
{
  ...
  if ((opcode = is_immed (op, r))) {
    ...
    if (strcmp(opcode, "add") == 0) {
      gen_inst ("mov %,%", REG_T3, lreg);
      gen_inst ("~ %,%", opcode, REG_T3, rreg);
      gen_inst ("mov %,%", reg, REG_T3);
    }
    else
...
void gen_expr_special (DECL *d, EXPRLIST *actuals)
{
  ...
  case SpecialWriteln_:{
    for ( ; actuals; actuals = actuals->rest) {
      ...
      gen_inst ("mov %,[%+8]", REG_T0, REG_FP);
      reg = get_expr_in_reg (actual, REG_T0);
      if (reg != REG_A0)
         //gen_inst ("move % %", REG_A0, reg);
         gen_inst ("mov [%+8],%", REG_FP, reg);
