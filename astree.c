...
PROCFUNCDEF *make_procfunc_def (DECLLIST *dl, IDENT *ret, BLOCK *blk)
{
...
{
  DECL *vd = make_decl(DeclVar_, id);
  vd->d.var = make_vardef(t);
  return vd;
}
...
DECL *make_procfunc_decl (IDENT *id, DECLLIST *dl, IDENT *ret, BLOCK *blk)
{
  DECL *d = make_decl(DeclProcFunc_, id);
  d->d.proc = make_procfunc_def(dl, ret, blk);
  return d;
}
...
FIELD *make_field (IDENT *fid, TYPE *ftyp)
{
  FIELD *f = anew (FIELD);
  f->field_id = fid;
  f->field_type = ftyp;
...
{
  TYPE *mary = make_type (TypeArray_);
  mary->t.arr = make_typearray (idx, elt);
  return mary;
}

TYPE *make_record_type(FIELDLIST *fl)
{
  TYPE *mrec = make_type (TypeRecord_);
  mrec->t.rec = make_fieldlist (0, fl);
  return mrec;
}
...
STMTIF *make_stmtif (EXPR *e, STMT *t, STMT *el)
{
...
STMTREPEAT *make_stmtrepeat (EXPR *e, STMTLIST *sl)
{
  STMTREPEAT *sr = anew (STMTREPEAT);
  sr->rpt_expr = e;
  sr->rpt_stmts = sl;
  return sr;
}

STMTFOR *make_stmtfor (IDENT *id, EXPR *fi, BOOLEAN up, EXPR *ft, STMT *fs)
{
  STMTFOR *sf = anew (STMTFOR);
  sf->for_name = make_binding (id);
  sf->for_init = fi;
  sf->for_upward = up;
  sf->for_to = ft;
  sf->for_stmt = fs;
...
STMT *make_if_stmt (EXPR *e, STMT *t, STMT *el)
{
  STMT *s = make_stmt (StmtIf_);
  s->s.ifx = make_stmtif (e, t, el);
  return s;
}

STMT *make_case_stmt (EXPR *e, CASELIST *cl)
{
...
STMT *make_repeat_stmt (EXPR *e, STMTLIST *sl)
{
...
STMT *make_for_stmt (IDENT *id, EXPR *fi, BOOLEAN up, EXPR *ft, STMT *fs)
{
  STMT *s = make_stmt (StmtFor_);
  s->s.forx = make_stmtfor (id, fi, up, ft, fs);
  return s;
}
